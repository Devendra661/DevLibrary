...
export default function IDCardPage() {
  ...
  useEffect(() => {
    // Call refreshUser to ensure the user object in store is up-to-date
    refreshUser();
      ...
      if (user && user.username) {
        try {
          ...
          if (response.ok) {
            ...
            toast.error("Failed to fetch student data: " + response.statusText);
          }
        } catch (error) {
  ...
  const displayUser = studentData || user;

  if (!displayUser) return <p>Loading...</p>;

  return (
    <div className="flex flex-col justify-center items-center min-h-[calc(100vh-64px-4rem)] p-6">
      {/* Heading + Quote */}
        ...
        >
          ...
          >
            ...
            <div className="mt-4 text-black px-6 flex flex-col gap-5 leading-tight">
              <p className="text-center text-2xl font-bold">{displayUser.studentName}</p>
              <p className="text-base"><FaIdCard className="inline mr-2" />ID No : {displayUser.studentId}</p>
              <p className="text-base"><FaPhone className="inline mr-2" />Phone : {displayUser.number}</p>
              <p className="text-base"><FaEnvelope className="inline mr-2" />E-mail : {displayUser.emailId || "email@example.com"}</p>
              <p className="text-base"><FaMapMarkerAlt className="inline mr-2" />Address : {displayUser.address}</p>
            </div>
